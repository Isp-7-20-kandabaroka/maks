...
def save_events_to_file():
    try:
        ...
        logging.error(f"Ошибка при сохранении данных о мероприятиях: {e}")
def load_events_from_file():
    try:
        with open("events_data.json", "r", encoding='utf-8') as file:
            return json.load(file)
    except (FileNotFoundError, json.JSONDecodeError) as e:
        logging.error(f"Ошибка при загрузке данных о мероприятиях: {e}")
        return {}
def save_password(password):
    try:
        ...
        logging.error(f"Ошибка при сохранении пароля: {e}")
...
async def process_password(message: types.Message, state: FSMContext):
    if message.text == Password:
        ...
        keyboard.add(types.InlineKeyboardButton("🔑 Сменить пароль", callback_data="change_password"))
...
async def go_back_to_start(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await cmd_start(callback_query.message, state)  # Возвращаем пользователя к начальному выбору
